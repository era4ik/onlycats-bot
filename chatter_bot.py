...
ADMIN_ID =7085368976  # <-- замени на свой Telegram ID
...
async def q4(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ["Утро ( 08–16 )", "Вечер ( 16–00 )", "Ночь ( 00–08 )"]
    ]
...
async def q5(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Где работал(а), сколько времени, какие были результаты и почему ушёл(ушла)?", reply_markup=ReplyKeyboardRemove())
    return Q6
...
async def q7(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Фанат пишет: “I can find free stuff online. Why should I pay you?” — твой ответ?", reply_markup=ReplyKeyboardRemove())
    return Q8
...
async def broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    users = sheet.col_values(2)  # колонка с user_id
    sent, failed = 0, 0

    for user_id in users[1:]:  # пропускаем заголовок
        try:
